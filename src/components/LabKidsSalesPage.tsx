...
import type { SupportedCurrency } from '../config/checkoutConfig';
...
export const LabKidsSalesPage: React.FC<LabKidsSalesPageProps> = ({
  ...
  const [purchaseNotification, setPurchaseNotification] = useState<{name: string, role: string, time: string} | null>(null);
  ...
  useEffect(() => {
    const buyers = [
      { name: 'Ana Silva', role: 'Mãe' },
      { name: 'Carlos Mendes', role: 'Pai' },
      { name: 'Juliana Costa', role: 'Educadora' },
      { name: 'Fernanda L.', role: 'Avó' },
      { name: 'Prof. Marcos', role: 'Educador' },
      { name: 'Roberto A.', role: 'Pai' },
    ];
    let index = 0;
    ...
    const showNext = () => {
      ...
      setPurchaseNotification({ ...buyer, time: `Há ${timeAgo} min` });
      index++;
    ...
    const initialTimeout = setTimeout(() => {
      showNext();
      setInterval(showNext, 180000); // A cada 3 minutos
    }, 15000); // 15 segundos após carregar
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-800 font-sans selection:bg-purple-600 selection:text-white pb-16">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="flex justify-center">
            <img
              src="/labkids/branding/mockup.webp"
      ...
      {/* 12. RODAPÉ (FOOTER)                                           */}
      {/* ------------------------------------------------------------- */}
      <footer className="bg-slate-900 text-slate-400 py-12 text-center text-xs sm:text-sm">
        <div className="max-w-4xl mx-auto px-4 space-y-4">
          <div className="font-black text-white text-lg">Lab Kids - Ciências & Descobertas</div>
          <div className="space-y-1">
            <p>E-mail de suporte: <a href="mailto:alssolucoesltda@gmail.com" className="text-purple-400 hover:underline">alssolucoesltda@gmail.com</a></p>
            <p>WhatsApp: <a href="https://wa.me/5516997327255" target="_blank" rel="noreferrer" className="text-purple-400 hover:underline">(16) 99732-7255</a></p>
          </div>
          <p className="text-[10px] leading-relaxed pt-6 border-t border-slate-800 text-slate-500">
          ...
          <p className="text-[10px]">&copy; {new Date().getFullYear()} Lab Kids. Todos os direitos reservados.</p>
        </div>
      </footer>

      {/* POPUP DE COMPRA (PROVA SOCIAL) */}
      <AnimatePresence>
        {purchaseNotification && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 left-6 z-50 bg-white rounded-2xl p-3 shadow-2xl border border-purple-100 flex items-center gap-3 w-64"
          >
            ...
            <div className="space-y-0.5">
              <p className="text-xs font-bold text-slate-800">
                {purchaseNotification.name} <span className="font-normal text-slate-500">({purchaseNotification.role})</span>
              </p>
              <p className="text-[10px] text-emerald-600 font-bold">Acesso Liberado • {purchaseNotification.time}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
