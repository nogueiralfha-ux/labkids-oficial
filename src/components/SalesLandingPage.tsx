...
export const SalesLandingPage: React.FC<SalesLandingPageProps> = ({ onEnterPlatform }) => {
  ...
  const handleSimulatePayment = async () => {
    try {
      // Chamando a nossa nova API Serverless (que conectará no Asaas)
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: "Luciana da Silva",
          email: "familia@toontaleskids.com",
          paymentMethod: "PIX",
          planId: selectedPlan
        })
      });
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          ...
          // Redireciona para a nova página de Obrigado!
          window.location.href = data.redirectUrl || '/obrigado';
        }, 1500);
      }
    } catch (error) {
  ...
  const getPlanDetails = (plan: PlanType) => {
    switch (plan) {
      ...
      case 'vitalicio':
      default:
        return { name: '👑 Plano Vitalício Família (Até 3 Filhos • 50% OFF + Temporada 5 + Módulo Trilíngue Inclusos)', price: '12x R$ 48,50 (ou R$ 485,00 à vista)' };
    }
  };
  ...
  const faqs = [
    {
      ...
      a: 'Os seus filhos poderão ouvir as histórias bíblicas em Português nativo, em Inglês (English Kids) e em Espanhol latino! É a forma mais natural e enriquecedora de desenvolver o ouvido bilíngue das crianças desde cedo com valores cristãos de alto nível. O Módulo Trilíngue completo está liberado como Super Bônus no Plano Vitalício!',
    },
    {
      q: 'Qual a diferença entre o Plano Pessoal e o Plano Familiar?',
      a: 'O Plano Pessoal foi feito para famílias com 1 filho (1 perfil individual). O Plano Familiar permite criar até 3 perfis simultâneos para que até 3 filhos possam ouvir, salvar favoritos e acompanhar seu próprio progresso de forma independente.',
    },
    {
      q: 'Como funciona o desconto de 50% nos planos anuais e no vitalício?',
      a: 'Na nossa promoção especial de lançamento, todos os planos anuais e o vitalício estão com 50% de desconto real aplicado. No Vitalício, de R$ 970,00 você paga apenas 12x de R$ 48,50 (ou R$ 485,00 à vista) uma única vez e garante todas as 4 temporadas prontas + a Temporada 5 (Heróis da Fé) + o Módulo Trilíngue sem nunca mais pagar mensalidade.',
    },
    {
    ...
    {
      q: 'Para qual faixa etária o Toon Tales Kids é recomendado?',
      a: 'A plataforma foi desenvolvida especialmente para crianças de 6 a 12 anos, mas crianças menores (a partir de 3 anos) também adoram ouvir as histórias na hora de dormir ou durante viagens em família.',
    },
    {
      q: 'Como funciona a Garantia de 7 Dias?',
      a: 'Você tem 7 dias completos para testar com seus filhos. Se por qualquer motivo achar que não valeu a pena, basta solicitar o cancelamento e devolveremos 100% do seu dinheiro na hora, sem burocracia.',
    },
  ];

  return (
    <div className="space-y-12 sm:space-y-16 select-none antialiased text-slate-800 pb-16">
      {/* 1. TOP ANNOUNCEMENT BAR */}
      <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-white py-2.5 px-4 text-center text-xs sm:text-sm font-black font-brand uppercase tracking-wider shadow-sm flex items-center justify-center gap-2 rounded-2xl">
        <Sparkles className="w-4 h-4 animate-spin" />
        <span>🎉 LANÇAMENTO TRILÍNGUE • 50% OFF EM TODOS OS PLANOS ANUAIS E NO PLANO VITALÍCIO!</span>
      </div>
      ...
      <div className="text-center space-y-4 max-w-4xl mx-auto pt-2">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-4 pt-1 text-xs font-bold text-slate-700">
          <span className="px-3.5 py-1.5 rounded-2xl bg-white border border-amber-200 shadow-2xs flex items-center gap-1.5">
            🌎 <strong>Trilíngue (PT • EN • ES)</strong>
          </span>
          <span className="px-3.5 py-1.5 rounded-2xl bg-white border border-amber-200 shadow-2xs flex items-center gap-1.5">
      ...
      <section className="relative rounded-[36px] overflow-hidden bg-slate-950 border-4 border-amber-400 shadow-2xl text-white">
        <div className="relative min-h-[460px] sm:min-h-[520px] flex flex-col justify-end p-6 sm:p-10 lg:p-14 overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl space-y-4">
            ...
            <div className="space-y-1">
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black font-brand tracking-tight text-white drop-shadow-md">
                {currentHero.title}
              </h1>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              >
                ...
                <span>Ver por Dentro (Vídeo)</span>
              </button>
      ...
      {/* 4. NOVA SEÇÃO: EDUCAÇÃO BÍBLICA TRILÍNGUE */}
      <section className="bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-950 border-4 border-indigo-500/40 rounded-[36px] p-8 sm:p-12 lg:p-16 text-white shadow-2xl space-y-8 relative overflow-hidden">
        ...
        <div className="text-center space-y-3 max-w-3xl mx-auto relative z-10">
          <span className="px-3.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-400/40 text-xs font-black uppercase tracking-wider font-brand flex items-center gap-1.5 w-fit mx-auto">
            ...
            Educação Bilíngue & Trilíngue Cristã
          </span>
          ...
          <p className="text-xs sm:text-sm text-indigo-200 leading-relaxed max-w-2xl mx-auto">
            Pesquisas neurológicas comprovam que crianças que ouvem histórias em outros idiomas desenvolvem maior concentração, memória e facilidade de pronúncia. Una a fé e o futuro do seu filho!
          </p>
        </div>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-8 space-y-4">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed max-w-2xl">
              ...
              Estamos gravando a maior e mais emocionante superprodução da história da plataforma: as vidas de <strong>Enoque, Jó, Gideão, Sansão, Daniel na Cova dos Leões, Elias e a Rainha Ester</strong> com orquestração ao vivo e elenco infantil completo!
            </p>
          ...
          <div className="lg:col-span-4 flex items-center justify-center">
            <div className="w-64 sm:w-72 aspect-[3/4] rounded-3xl overflow-hidden border-4 border-amber-300 shadow-2xl bg-slate-900 relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent flex flex-col justify-end p-5 text-center">
                ...
                <p className="font-brand font-black text-sm text-white">Abertura Oficial Liberada</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          {[
            ...
            },
            {
              ...
              desc: 'Ensina sobre transformação de vida, ousadia no evangelho e fé.',
              img: '/characters/paul.jpg',
              verse: 'Tudo posso naquele que fortalece. (Fp 4:13)',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white border border-amber-200 rounded-3xl p-6 shadow-sm space-y-3">
            ...
            <p className="text-xs sm:text-sm text-slate-600 italic leading-relaxed">
              "Meus filhos de 7 e 9 anos agora pedem para ouvir a história de Davi e Golias em inglês antes de dormir. O vocabulário deles cresceu muito!"
            </p>
      ...
      {/* 10. PRICING & PLANS TABLE (COM MÓDULO TRILÍNGUE & 50% OFF) */}
      <section id="planos" className="space-y-8 scroll-mt-20">
        <div className="text-center space-y-3 max-w-3xl mx-auto">
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Acesso imediato às 4 Temporadas completas + Bônus da Temporada 5 e Módulo Trilíngue no Plano Vitalício.
          </p>

          {/* Billing Cycle Switcher */}
          <div className="inline-flex items-center p-1.5 rounded-2xl bg-amber-100/80 border-2 border-amber-300 shadow-inner mt-2">
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch max-w-6xl mx-auto">
          ...
          <div className="bg-white border-2 border-slate-200 hover:border-amber-300 rounded-[32px] p-8 shadow-sm flex flex-col justify-between space-y-6 transition-all">
            <div className="space-y-4">
              ...
              <div>
                ...
                {billingCycle === 'annual' ? (
                  <div className="mt-2 space-y-0.5">
                    <span className="text-xs line-through text-slate-400">De R$ 238,80 por apenas</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl sm:text-4xl font-black text-slate-900 font-brand">12x R$ 9,90</span>
                    </div>
                    <span className="text-xs text-slate-500">ou R$ 119,00 à vista no PIX / ano</span>
                  </div>
                ) : (
                  <div className="mt-2 space-y-0.5">
                    <span className="text-xs text-slate-400">Cobrança Mensal Flexível</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl sm:text-4xl font-black text-slate-900 font-brand">R$ 19,90</span>
              ...
              <ul className="space-y-2.5 text-xs text-slate-600 font-medium pt-4 border-t border-slate-100">
                ...
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-emerald-500" /> 4 Temporadas Completas em Português
                </li>
                <li className="flex items-center gap-2">
            ...
            >
              {billingCycle === 'annual' ? 'Assinar Pessoal Anual (50% OFF)' : 'Assinar Pessoal Mensal (R$ 19,90)'}
            </button>
          </div>

          {/* Card 2: Plano Familiar (3 Filhos) */}
          <div className="bg-white border-2 border-amber-300 hover:border-amber-400 rounded-[32px] p-8 shadow-md flex flex-col justify-between space-y-6 transition-all relative">
            <div className="space-y-4">
              ...
              <div>
                ...
                {billingCycle === 'annual' ? (
                  <div className="mt-2 space-y-0.5">
                    <span className="text-xs line-through text-slate-400">De R$ 598,80 por apenas</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl sm:text-4xl font-black text-slate-900 font-brand">12x R$ 24,90</span>
                    </div>
                    <span className="text-xs text-slate-500">ou R$ 297,00 à vista no PIX / ano</span>
                  </div>
                ) : (
                  <div className="mt-2 space-y-0.5">
                    <span className="text-xs text-slate-400">Cobrança Mensal Família</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl sm:text-4xl font-black text-slate-900 font-brand">R$ 49,90</span>
                      <span className="text-xs text-slate-500"> / mês</span>
                    </div>
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Perfeito para famílias com 2 ou 3 crianças ouvirem ao mesmo tempo.
              </p>
              ...
              <ul className="space-y-2.5 text-xs text-slate-600 font-medium pt-4 border-t border-slate-100">
                ...
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-emerald-500" /> 4 Temporadas Prontas (34 Episódios)
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-emerald-500" /> Primeiros Áudios Bilíngues Liberados
                </li>
                <li className="flex items-center gap-2">
            ...
            >
              {billingCycle === 'annual' ? 'Assinar Familiar Anual (50% OFF)' : 'Assinar Familiar Mensal (R$ 49,90)'}
            </button>
          </div>

          {/* Card 3: Plano Vitalício Família (⭐ CAMPEÃO DE VENDAS COM TEMPORADA 5 + MÓDULO TRILÍNGUE) */}
          <div className="relative bg-gradient-to-b from-slate-950 via-amber-950 to-slate-950 text-white border-4 border-amber-400 rounded-[36px] p-8 shadow-2xl flex flex-col justify-between space-y-6 transform lg:-translate-y-4">
            ...
            <div className="space-y-4 pt-2">
              ...
              <div>
                <span className="text-xs line-through text-slate-400">De R$ 970,00 por apenas</span>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl sm:text-5xl font-black font-brand text-amber-400">12x R$ 48,50</span>
                </div>
                <span className="text-xs text-amber-200 font-bold">ou R$ 485,00 à vista no PIX (Taxa Única)</span>
              </div>
            ...
            >
              Comprar Vitalício com 50% OFF (R$ 485)
            </button>
          </div>
      ...
      {/* 14. VIDEO TOUR MODAL (30S SHOWCASE) */}
      <AnimatePresence>
        {isVideoModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base sm:text-lg font-black font-brand text-amber-300">
                      Tour Oficial • Conheça por Dentro da Plataforma (30s)
                    </h3>
                    <p className="text-[11px] text-slate-400">
                      Veja a experiência de áudio 3D, as 4 temporadas, quizzes e modo hora de dormir
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Interactive 30-Second Animated Video Tour */}
        ...
        {isCheckoutModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !checkoutSuccess && setIsCheckoutModalOpen(false)}
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 1, y: 0 }}
              className="relative z-10 w-full max-w-lg rounded-[32px] overflow-hidden bg-white border-4 border-amber-300 p-6 sm:p-8 shadow-2xl space-y-6"
            >
              {!checkoutSuccess ? (
                <>
                  <div className="text-center space-y-1.5 border-b border-slate-100 pb-4">
                    <span className="px-3 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-black uppercase font-brand">
                      🔒 Checkout 100% Seguro
                    </span>
                    <h3 className="text-xl sm:text-2xl font-black text-slate-900 font-brand">
                      Finalizar Inscrição
                    ...
                    <div className="p-3 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 font-bold">
                      <p className="font-brand font-black text-sm">{getPlanDetails(selectedPlan).name}</p>
                      <p className="text-orange-600 font-extrabold">{getPlanDetails(selectedPlan).price}</p>
                    </div>
                  </div>

                  <div className="space-y-3 text-xs font-bold text-slate-700">
                    <div>
                      <label className="block text-slate-500 mb-1">Nome do Responsável</label>
                      <input
                        type="text"
                        defaultValue="Luciana da Silva"
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-500 mb-1">E-mail para Acesso</label>
                      <input
                        type="email"
                        defaultValue="familia@toontaleskids.com"
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-500 mb-1">Forma de Pagamento</label>
                      <div className="grid grid-cols-2 gap-2">
                        <button className="py-2.5 rounded-xl bg-orange-500 text-white font-black text-xs">
                          PIX (Acesso Imediato)
                        </button>
                        <button className="py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-xs">
                          Cartão de Crédito
                        </button>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-black text-sm font-brand uppercase tracking-wider shadow-lg shadow-emerald-500/40 active:scale-95 transition-all"
                  >
                ...
                <div className="text-center py-8 space-y-4">
                  ...
                  <p className="text-xs text-slate-600">
                    Seu acesso ao <strong>Toon Tales Kids</strong> foi liberado. Entrando na plataforma...
                  </p>
                </div>
              )}
            </motion.div>
